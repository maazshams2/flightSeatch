...
Cypress.Commands.add('verifyFlightTileOrderByPrice', (sorter) => {
  let current = [];
  let next = [];
  var nextPrice;
  ...
  cy.flightTileList()
    .each(($el, index) => {
      if (cy.flightTileList().eq(index + 1).should('exist')){
        cy.wrap($el)
          .find('.index_price_2ibrS')
          .then((price) => {
            // var currentPriceValue = parseInt(utilFunctions.getPrefix(price.text(), '$'));
            current.push(parseInt(utilFunctions.getPrefix(price.text(), '$')));

            cy.flightTileList()
              .eq(index + 1)
              .find('.index_price_2ibrS')
              .then((field) => {
                // nextPrice = parseInt(utilFunctions.getPrefix(field.text(), '$'));
                next.push(parseInt(utilFunctions.getPrefix(field.text(), '$')));
              });
          });
        }
    });
  ...
  if (sorter == 'asc')
    cy.wrap(current).should('deep.eq', next);
    // expect(currentPriceValue).to.be.at.least(nextPrice);
  else if (sorter == 'desc')
    cy.wrap(current).should('deep.eq', next);
  // expect(currentPriceValue).to.be.at.most(nextPrice);

});

Cypress.Commands.add('verifyFlightTileOrderByDate', (sorter) => {
  cy.flightTileList()
    .each(($el, index) => {
      if (cy.flightTileList().eq(index + 1).should('exist')){
        cy.wrap($el)
          ...
          .eq(0)
          .then((date) => {
            var currentDateValue = utilFunctions.getPrefix(date.text(), ',');
            ...
            cy.flightTileList()
              ...
              .eq(0)
              .then((field) => {
                var nextDateValue = utilFunctions.getPrefix(field.text(), ',');

                if (sorter == 'asc'){
                  var currentDate = new Date(utilFunctions.convertToDateFrom(currentDateValue));
                  var nextDate = new Date(utilFunctions.convertToDateFrom(nextDateValue));
          
                  expect(currentDate).to.be.at.least(nextDate);
                }
                else if (sorter == 'desc'){
                  var currentDate = new Date(utilFunctions.convertToDateTo(currentDateValue));
                  var nextDate = new Date(utilFunctions.convertToDateTo(nextDateValue));
          
                  expect(currentDateValue).to.be.at.most(nextDate);
                }
              });
          });
        }
